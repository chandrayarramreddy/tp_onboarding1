...
function readyStateHandlerLoadText1(req,responseTextHandler){
    return function() {
        if (req.readyState == 4) {
            if (req.status == 200) {
                //(document.getElementById("loadingImage")).style.display = "none";
...
function populatePoDetails(responseXML)
{
    ...
    if(chk.childNodes[0].nodeValue =="true") {
        ...
        details = details +"<tr><td class='ajaxTd'>PO # :</td><td class='ajaxTd' onclick='purchaseOrderWebform("+poNUM+","+fileID+")'> <a style='cursor: pointer; cursor: hand;'>"+poNUM+"</a></td></tr>";
...
function populateInvDetails(responseXML)
{
    ...
    if(chk.childNodes[0].nodeValue =="true") {   
        ...
        var details="<table style='margin: 0 0 0 0;padding: 0px 0px;'>"+
        "<tr><td class='ajaxTd'>Instance Id :</td><td class='ajaxTd'>"+fileID+"</td></tr>"+
        "<tr><td class='ajaxTd'>Invoice # :</td><td class='ajaxTd' onclick='billingInvoiceWebform("+invNum+","+fileID+")'><a style='cursor: pointer; cursor: hand;'>"+invNum+"</a></td></tr>"+
        "<tr><td class='ajaxTd'>PO #:</td><td class='ajaxTd'>"+poNum+"</td></tr>"+
...
//purchaseOrderWebform

function purchaseOrderWebform(poNum,fileID) {
    // alert('fileId'+fileID);
    //alert('poNum'+poNum);
     var file_path="C:/EDIPROGRAMFILES/XML file/850.xml";
      var url="getpurchaseOrderForm.action?ponum="+poNum+"&instanceId="+fileID+"&file_path="+file_path;
       window.location=url;
}
//billingInvoiceWebform("+invNum+","+fileID+")
function billingInvoiceWebform(invNum,fileID) {
    // alert('fileId'+fileID);
    //alert('poNum'+invNum);
   var file_path="C:/EDIPROGRAMFILES/XML file/input.xml";
      var url="getBillingInvoiceWebform.action?invNum="+invNum+"&instanceId="+fileID+"&file_path="+file_path;
       window.location=url;
}
function onSave(){
   // alert('hi---');
   //var startDate=document.getElementById('add').value;
   //alert('startDate------- '+startDate);
    var r=confirm("Do you want to save this file!");
    if(r == true) {
        document.getElementById("webForm").action="doSaveFile.action";
...
function onSubmit(){
    ...
    var r=confirm("Do you want to save this file!");
    if(r == true) {
        document.getElementById("invoiceForm").action="doInvoiceSaveFile.action";
        document.getElementById("invoiceForm").submit();
    }
}